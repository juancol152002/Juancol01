...
class WalletBalanceSerializer(serializers.ModelSerializer):
    # Usamos 'nombrecripto' y 'preciousd' que son tus campos reales
    nombre_moneda = serializers.CharField(source='currency.nombrecripto') 
    simbolo = serializers.CharField(source='currency.simbolo')
    precio_actual = serializers.DecimalField(source='currency.preciousd', max_digits=20, decimal_places=2)
    valor_en_usd = serializers.SerializerMethodField()

    class Meta:
        model = Wallet
        fields = ['id', 'nombre_moneda', 'simbolo', 'balance', 'precio_actual', 'valor_en_usd']

    def get_valor_en_usd(self, obj):
        # Aquí también cambiamos .precio por .preciousd
        return round(obj.balance * obj.currency.preciousd, 2)